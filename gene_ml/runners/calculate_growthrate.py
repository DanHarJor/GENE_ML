...
def calculate_growthrate2(scanfiles_dir, suffix, calc_from_apar=False):
    ...
    par.Read_Pars(parameters_path)#'parameters_'+suffix)
    pars = par.pardict
    if pars['n_spec'] == 1:
        time, nrgi = get_nrg0(suffix,nspec=1, path=scanfiles_dir)
    elif pars['n_spec'] == 2:
        time, nrgi, nrge = get_nrg0(suffix,nspec=2, path=scanfiles_dir)
    elif pars['n_spec'] == 3:
        time, nrgi, nrge, nrg2 = get_nrg0(suffix,nspec=3, path=scanfiles_dir)
    else:
    ...
    time = np.empty(0)
    for i in range(len(field.tfld)):#(istart,iend):
    ...
    for t in ignore_times:
        tdif = np.abs(time-t)
        gamma = np.delete(gamma, [np.argmin(tdif)])
        time = np.delete(time, [np.argmin(tdif)])
    return gamma, time
